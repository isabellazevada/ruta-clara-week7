...
On the live URL, María can opt into a fictional demonstration, choose a simulated trip and see a route map with a braking event inferred from simulated phone accelerometer and GPS samples. The screen labels the model output **SIMULATED AI**, shows uncertainty and offers a large one-tap correction such as “Había peatón,” “Bache,” “Otro vehículo,” or “Fue un error.” Her correction changes that event to **context supplied / pending human review** and can be undone. She can leave the demo, view a plain-language use-of-data policy, and verify that no sanction, individual score, raw trace or personal record is created. The reviewer screen shows only aggregate counts; a context-supplied event remains unresolved pending human review. It cannot sanction anyone.
...
![Visual flowchart of the Ruta Clara demonstration, with opt-in, simulated braking alert, driver context and unresolved review state](diagrams/flowchart.png)

The visual version is included for reading and presenting. The Mermaid source below remains in the packet as required by the assignment.
...
![Visual swimlane showing the driver, prototype and proposed human-review responsibilities](diagrams/swimlane.png)
...
| Geodata and map | Schematic SVG map projected from invented latitude and longitude points, conspicuously labeled | Show coarse segments and event location; no external map tiles or live location; renders offline after the page loads |
| ML | Local logistic regression trained in JavaScript on twelve invented acceleration windows | Real local inference must execute; display “SIMULATED AI” and confidence as illustrative, never safety certification |
...
3. Verify the SVG route map retains the simulated label and coarse location; no raw driver trace or identifier is exported.
